...
// Replaces the old static "Good morning, Founder" hero copy. Time-of-day is read from the
// founder's own browser clock (not the server), which is the right zone for a greeting they'll
...
function founderHeroCopy(
  ...
  if (attentionItems.length === 0) {
    return {
      headline: `${greeting} — all clear right now`,
      subtext: "No urgent items today. Safe Mode is on and nothing needs your review."
    };
  }

  const count = attentionItems.length;
  return {
    headline: `${greeting} — ${count} thing${count === 1 ? "" : "s"} need${count === 1 ? "s" : ""} your review today`,
    subtext: attentionItems[0].text
  };
}
                ...
                <div className="approval-row">
                  ...
                  <div>
                    ...
                    <div className="approval-meta-grid">
                      ...
                      <MetricRow label="Created" value={formatLocalDateTime(row.createdAt)} />
                    </div>
...
function batchCountsOf(value: unknown): { updatedCount: number; skippedCount: number } {
  const root = recordOf(value);
  const nested = recordOf(root.result);
  return {
    updatedCount: readNumber(root.updatedCount ?? nested.updatedCount),
    skippedCount: readNumber(root.skippedCount ?? nested.skippedCount)
  };
}

// Splits a large id list into chunks of at most `size`. Both batch endpoints this feeds
// (action-ledger and listing-drafts) cap a single request's id array, so any bulk action fed
// by "Select All Loaded" -- which can easily exceed a page's worth of rows -- has to be sent
// across multiple requests rather than one.
      ...
      <div className="approval-card-head">
        <div className="approval-card-title-row">
          ...
          <div className="approval-title-block">
            ...
            <span>Created {formatLocalDateTime(row.createdAt)}</span>
          </div>
        </div>
...
function ApprovalCenterPage() {
  ...
  const [message, setMessage] = useState<{ type: "success" | "error"; text: string; at: string } | null>(null);
  function showMessage(type: "success" | "error", text: string): void {
    setMessage({ type, text, at: new Date().toISOString() });
  }
  ...
  function selectAllLoaded() {
    setSelectedIds(() => {
      ...
      rows.filter(isSelectableBatchAction).forEach((row) => next.add(row.id));
      return next;
    });
  }
  ...
  async function copyActionId(id: string) {
    try {
      ...
      showMessage("success", `Copied action ID ${formatShortId(id)} for debugging.`);
    } catch (error) {
      showMessage("error", `Copy failed: ${sanitizeActionError(error)}`);
    }
  }
  ...
  async function act(row: ActionLedgerRow, action: LedgerAction) {
    ...
    if (action === "approveExecute" || action === "approveExecuteListing" || action === "approveSavePassport") {
      ...
      try {
        ...
        showMessage("success", result?.message ?? `Processed action ${formatShortId(id)}.`);
      } catch (error) {
        showMessage("error", `Action failed: ${sanitizeActionError(error)}`);
      } finally {
        setProcessing(null);
      }
      return;
    ...
    try {
      ...
      showMessage("success", `${labelize(action)} saved for action ${formatShortId(id)}.`);
    } catch (error) {
      showMessage("error", `Action failed: ${sanitizeActionError(error)}`);
    } finally {
      setProcessing(null);
  ...
  async function reopenAction(row: ActionLedgerRow) {
    ...
    try {
      ...
      showMessage("success", `Action ${formatShortId(id)} reopened and moved back to Waiting for Approval.`);
    } catch (error) {
      showMessage("error", `Reopen failed: ${sanitizeActionError(error)}`);
    } finally {
      setReopeningId(null);
    }
  }
  ...
  async function batchAct(action: BatchLedgerAction) {
    ...
    if (action === "delete") {
      const confirmed = window.confirm(
        `Permanently delete ${ids.length} selected item${ids.length === 1 ? "" : "s"} from the Approval Center? ` +
        "This removes them from the database entirely and cannot be undone. Approved, Rejected, Monitoring, and " +
        "Completed history is never touched by this button."
      );
      if (!confirmed) return;
    }

    const actions: Record<BatchLedgerAction, { path: string; body: (chunkIds: string[]) => Record<string, unknown>; label: string }> = {
      reject: {
      ...
      delete: {
        path: "/api/action-ledger/batch/delete",
        body: (idsChunk) => ({
          sellerId: SELLER_ID,
          ids: idsChunk,
          note: "Permanently deleted in batch from Approval Center",
          actor: "founder"
        }),
        label: "Batch delete"
      },
    ...
    try {
      ...
      for (const chunk of chunks) {
        const response = await postJson<BatchActionResult>(actions[action].path, actions[action].body(chunk));
        const counts = batchCountsOf(response);
        updatedTotal += counts.updatedCount;
        skippedTotal += counts.skippedCount;
      }
      setSelectedIds(new Set());
      const refreshed = await refreshApprovalData();
      await refreshDailyPriorityRows(refreshed.rows);
      showMessage("success", `${actions[action].label} finished. Updated ${updatedTotal}, skipped ${skippedTotal}.`);
    } catch (error) {
      showMessage("error", `${actions[action].label} failed: ${sanitizeActionError(error)}`);
    } finally {
  ...
  async function loadDailyPriorities() {
    ...
    try {
      ...
      setSelectedIds(new Set());
      resetPagedView();
      showMessage("success", "Daily Priorities Mode loaded.");
    } catch (error) {
      showMessage("error", `Daily priorities failed: ${sanitizeActionError(error)}`);
    } finally {
      setBatchProcessing(null);
    }
  }
  ...
  async function dismissLowPriorityCostActions() {
    ...
    try {
      ...
      const refreshed = await refreshApprovalData();
      await refreshDailyPriorityRows(refreshed.rows);
      showMessage("success", `Low-priority cost actions dismissed. ${formatBatchCounts(response)}`);
    } catch (error) {
      showMessage("error", `Dismiss low-priority failed: ${sanitizeActionError(error)}`);
    } finally {
      setBatchProcessing(null);
    }
  }
  ...
  return (
    <div className="page">
      ...
      {message ? (
        <div className={`soft-state ${message.type === "error" ? "error-state" : "success-state"}`}>
          {message.text} <span className="soft-state-timestamp">({formatLocalDateTime(message.at)})</span>
        </div>
      ) : null}
        ...
        <>
          ...
          <div className="approval-pagination">
            ...
            <div className="button-row compact">
              ...
              <button type="button" className="secondary" onClick={selectAllLoaded} disabled={selectableLoadedCount === 0 || controlsDisabled}>
                Select All Loaded ({selectableLoadedCount})
              </button>
...
function ListingDraftsPage({ setActiveTab }: { setActiveTab: (tab: Tab) => void }) {
  ...
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  const [messageAt, setMessageAt] = useState("");
  function showMessage(text: string): void {
    setMessage(text);
    setMessageAt(text ? new Date().toISOString() : "");
  }
  ...
  const controlsDisabled = processingId !== "" || deleting;
  const selectedCount = selectedIds.size;

  useEffect(() => {
    setSelectedIds((current) => {
      const loadedIds = new Set(filteredRows.map((row) => row.id));
  ...
  async function generate() {
    ...
    try {
      ...
      showMessage("Listing drafts generated.");
      summary.reload();
      drafts.reload();
    } catch {
      showError("Could not generate listing drafts.");
    } finally {
      setProcessingId("");
    }
  }
  ...
  async function createAction(id: string) {
    ...
    try {
      ...
      showMessage("Approval action created for listing draft.");
      summary.reload();
      drafts.reload();
    } catch {
      showError("Could not create approval action.");
    } finally {
      setProcessingId("");
    }
  }
  ...
  async function deleteSelected() {
    ...
    const confirmed = window.confirm(
      `Permanently delete ${ids.length} selected draft${ids.length === 1 ? "" : "s"}? ` +
      "This removes the draft record(s) entirely and cannot be undone. Any already-approved or " +
      "already-sent action stays intact in the Approval Center either way -- this only removes " +
      "the draft content record itself."
    );
    if (!confirmed) return;
    ...
    try {
      ...
      let skippedTotal = 0;
      for (const chunk of chunks) {
        const response = await postJson<BatchActionResult>("/api/listing-drafts/batch/delete", {
          sellerId: SELLER_ID,
          ids: chunk
        });
        const counts = batchCountsOf(response);
        deletedTotal += counts.updatedCount;
        skippedTotal += counts.skippedCount;
      }
      setSelectedIds(new Set());
      showMessage(`Batch delete finished. Deleted ${deletedTotal}, skipped ${skippedTotal}.`);
      summary.reload();
      drafts.reload();
    } catch {
      showError("Could not batch delete listing drafts.");
    } finally {
      setDeleting(false);
    }
  }
        ...
        <div className="button-row">
          ...
          {message ? (
            <span className="save-message">
              {message} {messageAt ? <span className="save-message-timestamp">({formatLocalDateTime(messageAt)})</span> : null}
            </span>
          ) : null}
          {error ? (
            <span className="save-error">
              {error} {messageAt ? <span className="save-message-timestamp">({formatLocalDateTime(messageAt)})</span> : null}
            </span>
          ) : null}
        </div>
        <Card title="Drafts">
          ...
          {filteredRows.length > 0 ? (
            <div className="button-row compact">
              <button type="button" className="secondary" onClick={selectAllLoaded} disabled={controlsDisabled || filteredRows.length === 0}>
                Select All Loaded ({filteredRows.length})
              </button>
              <button type="button" className="secondary" onClick={clearSelection} disabled={controlsDisabled || selectedCount === 0}>
                Clear Selection
              </button>
              {selectedCount > 0 ? (
                <button type="button" className="danger-button" onClick={deleteSelected} disabled={controlsDisabled}>
          ...
          {drafts.loading ? <LoadingBlock /> : filteredRows.length === 0 ? <EmptyBlock text="No listing drafts match these filters." /> : (
            <div className="card-list command-card-list">
              {filteredRows.map((row) => (
                <article className={`item-card command-item-card ${selectedIds.has(row.id) ? "selected" : ""}`} key={row.id}>
                  <div className="item-top">
                    <label className="batch-checkbox" aria-label={`Select draft ${formatShortId(row.id)}`}>
                      <input
                        type="checkbox"
